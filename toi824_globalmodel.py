...
obj_id = "TIC 193641523"

filepath = '/home/u8015661/emma/toi824/scripts/'             #folder containing initialization files
out_folder = '/home/u8015661/emma/toi824/fitresults/'   #folder to store fit results

priorcsv = 'toi824_prior.csv'         #name of prior csv

#loading in data from csv file
data = pd.read_csv(filepath+'toi824_lc_cut.csv',comment='#', header=0) #reading in light curve file
...
BPmag = 11.383704
BPmagerr = .0314                       #use error floor of .03 for gaia bands
...
#read in rv data
jd_HARPS, rv_HARPS, erv_HARPS = np.loadtxt('TOI824_HARPS.vels', unpack=True, usecols=(0,1,2))
...
def initialize_model(planet_dict):
    ...
    time_base = 0.5*(jd_HARPS.min() + jd_HARPS.max())
...
def log_likelihood(theta, freeparams, fixedparams, system_list):
    ...
    rv_likelihood_value = rv_likelihood(system_list[1],system_list[0], jd_HARPS, rv_HARPS, erv_HARPS)
