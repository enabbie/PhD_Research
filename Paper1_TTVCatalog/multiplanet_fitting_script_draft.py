    ...
    for key in (system_list[i]):                            #and each parameter of that planet...
        if system_list[i][key].flag == 'free':              #decide if it's free (based on flag), and if so, add the NAME of the parameter to a list
            if i == 0:                                      #don't change the name of stellar parameters because they're shared
                ...
                temp = system_list[i][key].deepcopy
                temp.name = f'{key}{i}'
                freeparams.add(temp)
...
bf_model = np.zeros(t)
for i in range(len(system_list)-1):
        bf_model_i = batman_model(bf_system_list[i+1], bf_system_list[0],t) - 1   #model of system using best fit parameters
        bf_model += bf_model_i
bf_model += 1
...
#phase-folded plot with planet 1 best fit data
phase = phasefold(t,t01_bf,p1_bf)
...
plt.title("lightcurve of planet 1 with best fit model")
plt.xlim(-.1,.1)
plt.scatter(edited_phase,binned_flux, label = "binned data")
plt.plot(edited_phase, batman_model(bf_system_list[1],bf_system_list[0],t),c='red',label="model")
plt.legend()
...
#corner plot
p1,p2,t01,t02,u1,u2,rprs1,rprs2,b1,b2 = 10,5,0,0,.1,.1,.5,.5,.1,.1   #true values
fig = corner.corner(
    flat_samples, labels=labels, truths=[p1,p2,t01,t02,u1,u2,rprs1,rprs2,b1,b2]
);