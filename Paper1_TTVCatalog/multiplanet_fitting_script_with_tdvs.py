...
obj_id = "TIC 271772050"        #Make sure to change this to whatever star system you're fitting for
...
class Params:
    ...
    def initialize_theta(self):
        ...
        for x in self.data:
            ...
            position = locals()[initial_guess] + spread * np.random.randn(nwalkers, ndim)      #number of walkers, number of free parameters
            pos.append(position)
...
def batman_model(input_dict, star_dict, time):
    ...
    if (ecc > 0.8):
        ...
        if np.min(np.abs(np.ediff1d(time))) > 0.0007:               #supersample for long cadence data - if the minimum interval between time points is above 1 minute, then it's long cadence
    ...
    for i in range(len(system_list)-1):                             #for each planet,
        for tn in range(len(system_list[i+1])):                     #and each transit of that planet,
            for m in range(len(theta)):                             #and each free parameter that we go through,
                if (freeparams.unpack()[m][:2] == 't0') or (freeparams.unpack()[m][:1] == 'b'):              #if this is a transit-specific variable, we have to treat it differently
           
                    word = freeparams.unpack()[m]
                    var_name = word.split('_')[0]                   #omits everything after '_' -> ex. t01_3 becomes t01, b1_1 becomes b1 
...
def convert_to_variable(input_row):
    if (input_row['name'] == 't0') or (input_row['name']=='b'):
        ...
        var_dict  = TransitTimeVariable(input_row['name'],input_row['objectno'],input_row['value'],input_row['epoch'], input_row['error'], prior = var_prior, flag = input_row['flag'])
    else:
        var_prior = PriorFunction(input_row['prior'], center_value=input_row['value'], upper_bound= input_row['upper_bound'], lower_bound=input_row['lower_bound'])
...
def read_fit_param_csv(input_csv):
    ...
    for obj in range(nplanet+1):                                          #for each object in the system
        if obj == 0:                                                      #if it's a star, add to separate star dictionary
            ...
            exofop_data_m = exofop_data[exofop_data['objectno'] == obj]   #all parameters for specific planet + its different transit times and impact parameters
            ...
            list_name = f'planet{obj}_list'                               #'master' planet list that will hold all dictionaries for different transits of the same planet (different t0's and b's)
            locals()[list_name] = []
            ...
            for i in range(ntransits):                                #iterate through each transit to create transit dictionaries for a given planet
                ...
                exofop_data_b = exofop_data_m.drop(exofop_data_m[condition].index)     #drop all rows with transit times not pertaining to the one you want to look at

                b_condition = (exofop_data_m['name']=='b') * (exofop_data_m['epoch'] != epoch)
                exofop_data_i = exofop_data_b.drop(exofop_data_b[b_condition].index)   #drop all rows with irrelevant impact parameters
    ...
    for i in range(len(system_list)):                           #for each member of the system...
        if i == 0:                                              #add only one copy of the stellar parameters to be fit
            ...
            for transit in range(1,len(system_list[i])):         #iterate through each transit to add specific t0 and b -> t0 and b for each transit will be treated as its own free parameter
                epoch = system_list[i][transit]['t0'].epoch
                
                temp = deepcopy(system_list[i][transit]['t0'])
                temp.name = f't0{i}_{epoch}'

                temp_b = deepcopy(system_list[i][transit]['b'])
                temp_b.name = f'b{i}_{epoch}'
                
                if system_list[i][transit]['t0'].flag == 'free':
                    freeparams.add(temp)
                    true_values.append(temp)

                elif system_list[i][transit]['t0'].flag == 'fixed':
                    fixed_params.add(temp)
               
                else:
                    pass
                
                if system_list[i][transit]['b'].flag == 'free':
                    freeparams.add(temp_b)
                    true_values.append(temp_b)

                elif system_list[i][transit]['b'].flag == 'fixed':
                    fixed_params.add(temp_b) 
...
if __name__ == '__main__':
    ...
    theta_test, nwalkers, ndim, pos = freeparams.initialize_theta()
    ...
    #ndim = len(theta_test)
    #pos = theta_test + 1e-4 * np.random.randn(nwalkers, ndim)      #number of walkers, number of free parameters; box -> 20% of box spread; gaussian -> 10% of 1sigma width
